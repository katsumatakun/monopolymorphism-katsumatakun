...
public class FreeTicketBooth extends ChanceCard {
    ...
    public void chanceCardImplement(Player p, ChanceDeck cDeck) throws BankruptException{
        ...
        if (board[colorLoc(getColor())].getOwner() == ""){
            board[colorLoc(getColor())].setOwner(p);
            p.endTurn();
        }
        //if property numbered 2 has not been owned
        else if ((board[colorLoc(getColor())+1].getOwner() == "")){
            board[colorLoc(getColor())].setOwner(p);
            p.endTurn();
        }
            ...
            if (board[colorLoc(getColor())].getOwner() != p.getName()&& board[colorLoc(getColor())].getOwner() == board[colorLoc(getColor())+1].getOwner()){
            ...
            else if (board[colorLoc(getColor())].getOwner() == p.getName())
            {
                board[colorLoc(getColor())+1].setOwner(p);
                p.endTurn();
            }
            //Player has No.2 and another player has No.1
            else if (board[colorLoc(getColor())+1].getOwner() == p.getName()){
                board[colorLoc(getColor())].setOwner(p);
                p.endTurn();
            }
